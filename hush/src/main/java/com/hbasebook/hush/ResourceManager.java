 ...
 * <br/>
 * Using the class should be facilitated invoking <u>only</u> <code>getInstance()
...
public class ResourceManager {
  ...
  /**
   * Internal constructor, called by the <code>getInstance()</code>
   * methods.
   *
   * @param conf  The HBase configuration to use.
   * @throws IOException When creating the remote HBase connection fails.
  ...
  /**
   * Delayed initialization of the instance. Should be called once to
   * set up the counters etc.
   *
  ...
  /**
   * Returns a single table from the shared table pool. More convenient to
   * use compared to <code>getTablePool()</code>.
   *
  ...
  /**
   * Returns the previously retrieved tanle to the shared pool. The caller
   * must take care of calling <code>flushTable()</code> if there are any
   * pending mutatioons.
   *
   * @param table  The table reference to return to the pool.
   * @throws IOException When there is an issue talking to HBase.
   */
  ...
  /**
   * Convenience method to retrieve a new short Id. The value is returned
   * in the proper format to be used as a row key in the HBase table. Each
   * call increments the counter by one.
   *
   ...
   * Convenience method to retrieve a new short Id. The value is returned
   * in the proper format to be used as a row key in the HBase table. Each
   * call increments the counter by the given <code>incrBy</code>.
   *
   * @return The newly created short Id.
